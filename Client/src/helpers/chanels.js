  ...
  if (chatId) {
    ...
    try {
      if (newChatId && userId) {
        ...
        const channel = chatClient.channel("messaging", `myBestChat-${newChatId}`, {
          members: members,
          name: name,
          description:
            file && file.createdAt
...
const getUnreadMessagesForChat = async (chatsInfo, fileID, userID) => {
  try {
    ...
    const chat = chatsInfo.find((chat) => chat.chatId === `myBestChat-${myChat.id}`);
...
const updateChatDescriptionForFile = async (chatClient, file, owner) => {
  try {
    ...
    await chatClient.channel("messaging", `myBestChat-${chatData.id}`).update({
      name: file.name,
      description:
        file && file.createdAt
          ? `Type: ${file.type}, Status: ${file.status}, Created: ${file.createdAt}, Owner: ${owner}, Remark: ${file.remark}`
